var addnew = angular.module('addnew',[]);


	app.directive('myEnter', function () {
	    return function (scope, element, attrs) {
	        element.bind("keypress", function (event) {
	            if(event.which === 13) {
	                 scope.$apply(function (){
	                  scope.$eval(attrs.myEnter);
	                });

	                event.preventDefault();
	            }
	        });
	    };
	});


addnew.controller('addContactController', function(contact,$timeout,$scope,$location,$window,$http,$rootScope,$localStorage,$sessionStorage) {
	
	if($window.localStorage.length == 0)
 	{
 		if($location.path() == '/add_new'){
 			$timeout(function () {
 					$location.path('/')
	 		    }, 200)
 		}
 		
 	}

	$scope.tager=[];
	var uid=localStorage.getItem('id');


	  $scope.thumbnail = {
        dataUrl: ''
    };
    $scope.fileReaderSupported = window.FileReader != null;
    $scope.photoChanged = function(files){
     var ext = files[0].name.match(/\.(.+)$/)[1];
       if(angular.lowercase(ext) ==='jpg' || angular.lowercase(ext) ==='jpeg' || angular.lowercase(ext) ==='png'){
	        
	        if (files != null) {
	            var file = files[0];
	        	if ($scope.fileReaderSupported && file.type.indexOf('image') > -1) {
	           		$timeout(function() {
	                	var fileReader = new FileReader();
	                	fileReader.readAsDataURL(file);
	                	fileReader.onload = function(e) {
	                    	$timeout(function(){
								$scope.thumbnail.dataUrl = e.target.result;
	                    	});
	                	}
	            	});
	        	}
	    	}
	    	else
	    	{
	    		console.log('dd')
	    	}

       }  
       else{
       $scope.fileerror="Please Select Image only";
       $scope.thumbnail.dataUrl="assets/images/name.png";
      }      
    };
		
	$scope.submitContact = function(isValid) {
		// check to make sure the form is completely valid
		if (isValid) { 
			
			$location.path('/contacts')
			
			$scope.form.user_id=uid;
			$scope.form.tags = $scope.tager;
			
			// Code for by default base64 if contact doest not contain any photo
			if($scope.form.photo==null)
			{
			 	$scope.form.base64='null';
			 	$scope.form.photo='';
			}
			
			//else uploaded pic base64 goes here
			else
			{
			 	$scope.form.photo=$scope.form.photo.base64;
			 	if($scope.thumbnail.dataUrl=="assets/images/name.png")
				{
					$scope.form.base64='null';
					$scope.form.photo='';
				}
			}
			var form=$scope.form;
		    contact.addnewcontact(form);
		     contact.allcontacts().then(
		    	function(response) {
		    	$scope.mycard = response.data;

 		    });
 		};
	};

	//Funtion to go back to main Page 
	$scope.goback = function(){
			$location.path('/contacts');
	};

	//Funtion for giving suggestion on keyup of Tags Textfield(Yet to Implement)
	$scope.sugg=function(){
		
		var data = {
			'suggtag' : $scope.form.tags
			}
		var len=$scope.form.tags.length
		console.log(len)
		if(len == 0)
		{
				$scope.suggestion=false;
		}
		else
		{
				$scope.suggestion=true;	
    	}
    	contact.tagsuggestions(data)
	    	   .then(function(response)
	    	    {
	    			$scope.suggtag=response.data;
	    			$rootScope.suggestion=true;
				});
	    $scope.checkIf = function(allTags, tags){
               for (var i = 0; i < allTags.length; i++) {
                   if (allTags[i].id == tags.id) return allTags[i];
               }
               return false;
           }
	    $scope.checkIfName = function(allTags, tags){
               for (var i = 0; i < allTags.length; i++) {
                   if (allTags[i].name == tags.name) return allTags[i];
               }
               return false;
           }

		$scope.sel=function(tags){
			 var index = $scope.checkIf($scope.tager, tags);
			 if(index == false){			
			$scope.tager.push(tags);
			$rootScope.suggestion=false;
			}
		}


	    $scope.removeTag = function( tags){
               for (var i = 0; i < $scope.tager.length; i++) {
                   if ($scope.tager[i].id == tags.id){
                   	$scope.tager.splice(i,1);
                   } ;
               }
               return false;
           }


	}



		

	$scope.newtag=function(tags){
		if(tags != '' && tags != undefined){
	var id = $rootScope.genID();
	 var x={
	 	'id' : id,
	 	'name' : tags
	 };
			 var index = $scope.checkIfName($scope.tager, x);
			 if(index == false){
	 var index2 = $scope.sel(x);
}

	}
}

	$rootScope.genID = function(){
 return Math.floor(Math.random() * (9999999 - 1 + 1)) + 1;	}
});


