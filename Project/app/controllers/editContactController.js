var editcon = angular.module('edit',[]);

editcon.directive('myEnter', function () {
	    return function (scope, element, attrs) {
	        element.bind("keypress", function (event) {
	            if(event.which === 13) {
	                 scope.$apply(function (){
	                  scope.$eval(attrs.myEnter);
	                });

	                event.preventDefault();
	            }
	        });
	    };
	});
// create angular controller
editcon.controller('editContactController', function(contact,$timeout,$routeParams,$scope,$location,$window,$http,$rootScope,$localStorage,$sessionStorage){
	if($window.localStorage.length == 0)
 	{
 		if($location.path() == '/edit_contact'){
 			$timeout(function () {
 					$location.path('/')
	 		    }, 200)
 		}
 		
 	}
	
	$scope.tagArray =[];
	var id = $routeParams.id;

	$scope.thumbnail = {
        dataUrl: ''
    };
    $scope.fileReaderSupported = window.FileReader != null;
    $scope.photoChanged = function(files){
    $rootScope.form = {
    photo  : false
  	}
  	
  	if(files.length==0)
    {
    	$scope.card.photo = true;
    }
    var ext = files[0].name.match(/\.(.+)$/)[1];

	 	if(angular.lowercase(ext) ==='jpg' || angular.lowercase(ext) ==='jpeg' || angular.lowercase(ext) ==='png')
	  	{
		    if (files != null) {


	        var file = files[0];
	        	if ($scope.fileReaderSupported && file.type.indexOf('image') > -1) {
	           		$timeout(function() {
	                	var fileReader = new FileReader();
	                	fileReader.readAsDataURL(file);
	                	fileReader.onload = function(e) {
	                    	$timeout(function(){
								$scope.thumbnail.dataUrl = e.target.result;
	                    	});
	                	}
	            	});
	        	}
	    	}
		}
		else
		{
			$scope.fileerror="Please Select Image only";
	       $scope.thumbnail.dataUrl="assets/images/name.png";
		}    	 	
    }

	contact.detail(id);
	contact.detail(id).then(function(response){
    		var obj=[];
	       var currentContactid=response.data['id'];
            $scope.mycard = response.data;
	        $scope.card = $scope.mycard;

	        var obj1=localStorage.getItem('mycard');

	        //Getting all contact through localstorage
	       
	        var allcontacts=JSON.parse(obj1);
			var allmycontacts = [];
	       	angular.forEach(allcontacts, function(values, keys) {
	       		allmycontacts.push(values);
	       	});

	       angular.forEach(allmycontacts, function(value, key) {
 			 // this.push(key + ': ' + value);
			   if(currentContactid == value.id){
				   // localStorage.setItem('previous', JSON.stringify(response.data));
					var p = parseInt(key) - 1;
					var n = parseInt(key) + 1;
	    			$scope.nextItem = allmycontacts[n];
	    			$scope.previousItem = allmycontacts[p];
  				}
			});
	});


    $scope.movePrevious = function() 
    {

	    angular.forEach($scope.addnew, function(value, key) 
	    {
			if(key[0] == '$') return;
			if(value.$pristine == false)
			{ 	
				console.log(value.$pristine)
			  	if ($window.confirm("Do you want to update this contact?"))
				{
					if($scope.result = "Yes")
					{

							$scope.card.id=	$routeParams.id;
		    				$scope.card.userid=localStorage.getItem('id');
		    				$scope.card.updatetag=$scope.card.tags;
		    				if($scope.card.upphoto==null)
							{
							 	
							 	//$scope.card.base64='null';
							 	$scope.card.upphoto="";
							}
							
							//else uploaded pic base64 goes here
							else
							{
							 	$scope.card.photo=$scope.card.upphoto.base64;
							}
						    var c_id = 	$routeParams.id;
							console.log(c_id,$scope.card)
							var form = $scope.card;
							console.log(form)
						 	contact.update(c_id,form);
						 	var s = window.location.hash;
							s = s.substring(0, s.indexOf('edit_contact/'));

					    	window.location = window.location.pathname+s+'edit_contact/'+$scope.previousItem.id;
							
					}
				}	
				else if($scope.result = "No")
				{
					
		    	    var s = window.location.hash;
					s = s.substring(0, s.indexOf('edit_contact/'));

	    			window.location = window.location.pathname+s+'edit_contact/'+$scope.previousItem.id;
				}
			}
			else
			{
				var s = window.location.hash;
				s = s.substring(0, s.indexOf('edit_contact/'));
		    	window.location = window.location.pathname+s+'edit_contact/'+$scope.previousItem.id;	
			}
		});
    }

    $scope.moveNext = function() 
    {
    	angular.forEach($scope.addnew, function(value, key) {
			if(key[0] == '$') return;
			if(value.$pristine == false)
			{ 	
				console.log(value.$pristine)
			  	if ($window.confirm("Do you want to update this contact?"))
				{
					if($scope.result = "Yes")
					{

							$scope.card.id=	$routeParams.id;
		    				$scope.card.userid=localStorage.getItem('id');
		    				$scope.card.updatetag=$scope.card.tags;
						    var c_id = 	$routeParams.id;

						    if($scope.card.upphoto==null)
							{
							 	
							 	//$scope.card.base64='null';
							 	$scope.card.upphoto="";
							}
							
							//else uploaded pic base64 goes here
							else
							{
							 	$scope.card.photo=$scope.card.upphoto.base64;

							}

							console.log(c_id,$scope.card)
							var form = $scope.card;
							console.log(form)
						 	contact.update(c_id,form);
							var s = window.location.hash;
							s = s.substring(0, s.indexOf('edit_contact/'));
					    	window.location = window.location.pathname+s+'edit_contact/'+$scope.nextItem.id;						
					}
				}	
				else if($scope.result = "No")
				{
					
			    	var s = window.location.hash;
					s = s.substring(0, s.indexOf('edit_contact/'));

			    	window.location = window.location.pathname+s+'edit_contact/'+$scope.nextItem.id;
				}
			}
			else
			{
				var s = window.location.hash;
				s = s.substring(0, s.indexOf('edit_contact/'));
			    window.location = window.location.pathname+s+'edit_contact/'+$scope.nextItem.id;		
			}
		});
    }

		
	// Funtion on click of Update Contact
    $scope.updateContact = function() {		  

	    $scope.card.id=	$routeParams.id;
	    $scope.card.userid=localStorage.getItem('id');
	    $scope.card.updatetag=$scope.card.tags;
	    // Code for by default base64 if contact doest not contain any photo
		if($scope.card.upphoto==null)
		{
		 	
		 	//$scope.card.base64='null';
		 	$scope.card.upphoto="";
		}
		
		//else uploaded pic base64 goes here
		else
		{
		 	$scope.card.photo=$scope.card.upphoto.base64;
	 		if($scope.thumbnail.dataUrl=="assets/images/name.png")
		 	{
		 		$scope.card.base64='null';
		 	$scope.card.photo='';

		 	}
		}
		var c_id = 	$routeParams.id;
		console.log(c_id,$scope.card)
		var form = $scope.card;
	 	contact.update(c_id,form);
	 	contact.detail(id)
	    	.then(
	    		function(response) {
	      		$location.path('/contacts');
	            $scope.mycard = response.data;
		    });	           	  					
	}

	// Funtion on onclick of GOback
	$scope.goback = function(){
			$location.path('/contacts');	
	}		

	// FUnction on click of delete contact without warning
	$scope.delete=function(contactid){
		contact.deletecontact(contactid.id);
		contact.deletecontact(contactid.id)
		.then(
			function(response) {

			$window.alert("Your contact is deleted");

			console.log(response.data == "" || response.data == undefined)
			{
	      		$location.path('/contacts');	
			}
				
			var s = window.location.hash;
		s = s.substring(0, s.indexOf('edit_contact/'));

    	window.location = window.location.pathname+s+'edit_contact/'+$scope.nextItem.id;
		});	
	};

	
	$scope.sugg=function(){
		var data = {
			'suggtag' : $scope.card.name
			}

		var len=$scope.card.name.length
		console.log(len)
		if(len == 0)
		{
				$scope.suggestion=false;
		}
		else
		{
				$scope.suggestion=true;	
    	}
    		
		contact.tagsuggestions(data)
		.then(function(response){
    		$scope.suggtag=response.data;
    		$rootScope.suggestion=true;
		});


    	    $scope.checkIf = function(allTags, tags){
                   for (var i = 0; i < allTags.length; i++) {
                       if (allTags[i].id == tags.id) return allTags[i];
                   }
                   return false;
               }
    	    $scope.checkIfName = function(allTags, tags){
                   for (var i = 0; i < allTags.length; i++) {
                       if (allTags[i].name == tags.name) return allTags[i];
                   }
                   return false;
               }

		$scope.sel=function(tags){
			 var index = $scope.checkIf($scope.card.tags, tags);
			 if(index == false){			
			$scope.card.tags.push(tags);
			$rootScope.suggestion=false;
			}
			$rootScope.suggestion=false;	
		}
		


	}
	
	$scope.removeTag = function(x){
			       for (var i = 0; i < $scope.card.tags.length; i++) {
                       if ($scope.card.tags[i].id == x.id){
                       	$scope.card.tags.splice(i,1);
                       } ;
                   }
                   return false;
         }



	$scope.newtag=function(tags)
	{
		if(tags != '' && tags != undefined)
		{
			var id = $rootScope.genID();
			 var x=
			 {
			 	'id' : id,
			 	'name' : tags
	 		};
			var index = $scope.checkIfName($scope.card.tags, x);
		 	if(index == false)
			{
				var index2 = $scope.sel(x);
			}
		}
	}
	
	$scope.hideList=function()
	{
		$rootScope.suggestion=false;
	}	

	$rootScope.genID = function()
	{
 		return Math.floor(Math.random() * (9999999 - 1 + 1)) + 1;	
 	}
	
});
